...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc_grapheme_root = nn.Linear(512 * block.expansion, 168)
        self.fc_vowel_diacritic = nn.Linear(512 * block.expansion, 11)
        self.fc_consonant_diacritic = nn.Linear(512 * block.expansion, 7)
    ...
    def _forward_impl(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x1 = self.fc_vowel_diacritic(x)
        x2 = self.fc_grapheme_root(x)
        x3 = self.fc_consonant_diacritic(x)

        return {
            "logit_vowel_diacritic": x1,
